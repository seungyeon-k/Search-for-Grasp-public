...
def expSO3(so3):
    ...
    is_theta_not_zero = (theta > 1e-6).squeeze()
    w = torch.zeros(nBatch, 3).to(so3)
    w[is_theta_not_zero, :] = so3vec[is_theta_not_zero] / theta[is_theta_not_zero]
    wmat = skew_so3(w)
    expso3 = so3.new_zeros(nBatch, 3, 3).to(so3)
